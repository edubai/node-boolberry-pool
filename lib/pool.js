...
function jobRefresh(callback)
{
    ...
    getBlockTemplate(function(error, result){
        ...
        if (!currentBlockTemplate || result.height > currentBlockTemplate.height /*|| !last_block_sent_time || Date.now() - last_block_sent_time*/ )
        {
            ...
            if (miner.longPoll) {
                miner.fetchAddms(function () {
                    if (miner.longPoll) {
                        ...
                        log('debug', 'processBlockTemplate', 'GETJOB: reply1, job: ' + JSON.stringify(job));
    ...
    switch(method){
        ...
        case 'getjob':
            ...
            if (!longPollingEnabled || miner.protocol === 'tcp'){
                ...
                {
                    var job = miner.getJob();
                    log('debug', componentName, 'GETJOB: reply1, job: ' + JSON.stringify(job));
                    sendReply(null,
                        {
                            blob: job.blob,
                            job_id: job.job_id,
            ...
            miner.longPoll = {
                timeout: setTimeout(function(){
                    ...
                    {
                        var job = miner.getJob();
                        log('debug', componentName, 'GETJOB: reply2' );
                        sendReply(null, {
                            blob: job.blob,
                            job_id: job.job_id,
                            target: job.target,
            ...
            log('debug', componentName, 'GETJOB: reply3' );
            var job = miner.getJob();
            sendReply(null, {
                blob: job.blob,
                job_id: job.job_id,
